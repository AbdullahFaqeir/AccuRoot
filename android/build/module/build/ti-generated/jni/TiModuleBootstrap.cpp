...
#define TAG "com.accusolutions.root"
...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "com.accusolutions.root TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "com.accusolutions.root TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
Java_com_accusolutions_root_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.accusolutions.root", &TiModuleBinding);
